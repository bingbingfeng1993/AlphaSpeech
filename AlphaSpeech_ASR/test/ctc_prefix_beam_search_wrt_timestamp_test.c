...
int main() {
    ...
    CTC_Prefix_Beam_Search_WRT_Timestamp *ctc_decoder = ctc_prefix_beam_search_wrt_timestamp_create(abs_time_step, vocab_size, blank, first_beam_size, second_beam_size);

    int *result = (int *)malloc(sizeof(int)*abs_time_step);
    int *result_times = (int *)malloc(sizeof(int)*abs_time_step);
    int cont_time_step = 0;
    int best_prefix_dynamic_size = ctc_prefix_beam_search_wrt_timestamp_process(ctc_decoder, logp, result, result_times, cont_time_step);

    // load dict
	char dict_path[] = "../dict/lang_char_cn.txt";
